...
let availableSounds = [
  ...
  // new Audio('https://www.dropbox.com/s/gllotxkhr5nadhy/sound_02.mp3?raw=1'),
  // new Audio('https://www.dropbox.com/s/jm6gbsmdac1iepa/sound_03.mp3?raw=1'),
];

availableSounds[0].volume = 1; // I don't wanna r*pe your ears
...
function toggleSpecialTileType() {
  ...
  if (
    hoverTileY >= 0 &&
    hoverTileY < gridHeight &&
    hoverTileX >= 0 &&
    hoverTileX < gridWidth
  ) {
...
function mouseClick() {
  // console.log(`Clicked at: ${mousePosition.x}, ${mousePosition.y}`)
}
...
function update(elapsed) {
  ...
  if (appState === states_editor) {
    ...
    // to increase difficulity we will have to validate
    // the end reach of the cursor as well
    //    right and right bottom
...
function setCurrentTile(tileType) {
  if (tileType === _wall_tile || tileType === _path_tile) {
    if (gameExitTileX === hoverTileX && gameExitTileY === hoverTileY) {
      gameExitTileX = -1;
      gameExitTileY = -1;
    }
    if (gameStartTileX === hoverTileX && gameStartTileY === hoverTileY) {
      gameStartTileX = -1;
      gameStartTileY = -1;
    }
  }
  if (tileType === _start_tile) {
    ...
    if (gameStartTileX !== -1 && gameStartTileY !== -1) {
      tileMap[gameStartTileY][gameStartTileX] = _path_tile;
    }
    if (gameExitTileX === hoverTileX && gameExitTileY === hoverTileY) {
      gameExitTileX = -1;
      gameExitTileY = -1;
  ...
  if (tileType === _end_tile) {
    // remove existing exit point
    if (gameExitTileX !== -1 && gameExitTileY !== -1) {
      tileMap[gameExitTileY][gameExitTileX] = _path_tile;
    }
    if (gameStartTileX === hoverTileX && gameStartTileY === hoverTileY) {
      gameStartTileX = -1;
      gameStartTileY = -1;
...
function renderGameUI() {
  ...
  ctx.fillText(
    "\uF11e",
    gameExitTileX * tileWidth + gridStartX + 1,
    gameExitTileY * tileHeight + gridStartY + (tileHeight - 4)
  );
}
...
function renderEditorUI() {
  ...
  ctx.fillText(
    `You will need to have one Start point and one Exit point`,
    x,
    (y += 20)
  );
  ...
  ctx.fillText(
    `Start and Exit points can easily be plotted with a right-click`,
    x,
    (y += 20)
  );
  ctx.font = "12pt Calibri";
  ...
  ctx.fillText(`  1: Path`, x, (y += 20));
  ctx.fillStyle = "white";
  if (drawType === 2) ctx.fillStyle = "red";
  ctx.fillText(`  2: Start/Entry Point`, x, (y += 20));
  ctx.fillStyle = "white";
  if (drawType === 3) ctx.fillStyle = "red";
  ctx.fillText(`  3: End/Exit Point`, x, (y += 20));
  ctx.fillStyle = "white";
  if (isCtrlDown === true) ctx.fillStyle = "red";
  ctx.fillText(`Hold CTRL to draw walls`, x, (y += 40));
  ctx.fillStyle = "white";
  if (pathError && pathError.length > 0) {
    ctx.fillStyle = "rgba(192, 57, 43,1.0)";
    fillTextWrapped(pathError, x, (y += 20), 300, 20);
  }
}
  ...
  for (let y = 0; y < tileMap.length; ++y) {
    ...
    for (let x = 0; x < tileMap[y].length; ++x) {
      if (column !== tileMap[y][x]) {
        if (column !== -1) {
          rowData +=
            columnCount === 1 ? `, ${column}` : `, ${columnCount}x${column}`;
        }
        column = tileMap[y][x];
        columnCount = 1;
      } else columnCount++;
...
function decodeMapData(str) {
  ...
  let output = [];
  let index = 0;
  let width = parseInt(data[index++]);
  let height = parseInt(data[index++]);
  let imageIndex = parseInt(data[index++]);
  let soundIndex = parseInt(data[index++]);
  let startX = 0;
  let startY = 0;
  let exitX = 0;
  let exitY = 0;
  // 2-pass, this can be optimized to use 1-pass
  // but its easier to follow with the 2-pass :P
  let tileData = [];
  for (let i = index; i < data.length; ++i) {
    let spec = data[i].split("x");
    if (spec.length === 2) {
      // has tile counter
      let count = parseInt(spec[0]);
      let tile = parseInt(spec[1]);
  ...
  // console.log("Decoded map: " + JSON.stringify(output))
  return {
    width: width,
    height: height,
...
function getTileName(tile) {
  if (tile === 0) return `Wall (${tile})`;
  if (tile === 1) return `Path (${tile})`;
  if (tile === 2) return `Start (${tile})`;
  if (tile === 3) return `Exit (${tile})`;
  return "??";
}

function toScreenPoint(worldPos) {
  return {
    x: worldPos.x * tileWidth + gridStartX,
    y: worldPos.y * tileHeight + gridStartY,
  };
}

function validateMaze(onSuccess, onFail) {
  // make sure we can go from Start to End
  // first ensure we have both start and end points
  let start = getStartPoint();
  let exit = getExitPoint();
  if (start.x === -1 || start.y === -1) {
    onFail("You have no starting point!!!");
    return;
  }
  if (exit.x === -1 || exit.y === -1) {
    onFail("You have no exit point!!!");
    return;
  }
  // you can only move UP, DOWN, LEFT and RIGHT
  // so diagonal paths does not work
  ...
  easyStar.findPath(start.x, start.y, exit.x, exit.y, (path) => {
    if (path && path.length > 0) onSuccess();
      ...
      onFail(
        "The exit point is unreachable!!! Make sure you have a clear straight path, diagonal lines does not work."
      );
  });
  easyStar.calculate();
}
  ...
  if (appState === states_preview) {
    ...
    validateMaze(
      () => {
        ...
        prepareGame();
        document.querySelector(".btn-create-own").style.display = "none";
      },
      (err) => alert(err)
    );
  }
...
function prepareGame() {
  ...
  document.querySelector(".btn-create-own").style.display = "";
...
function gameOverLose() {
  ...
  gameStartBtn.style.display = "block";
  console.log("Awwes! you didnt make it this time.");
}

function gameOverWin() {
  gameWinImage.src = gameWinImageUrl;
  gameState = gameStates_notStarted;
  gameWinSound.play();
...
function play() {
  ...
  if (!data || data.length == 0) {
    if (confirm("Good luck bro! I love you") !== true) return;

    data = `32, 32, 0, 0, 32x0, 32x0, 3x1, 29x0, 2, 6x1, 25x0, 7x1, 25x0, 4x0, 3x1, 25x0, 5x0, 3x1, 24x0, 5x0, 
                3x1, 4x0, 3x1, 17x0, 6x0, 3x1, 2x0, 4x1, 17x0, 6x0, 6x1, 2x0, 2x1, 16x0, 8x0, 4x1, 2x0, 3x1, 15x0, 
                14x0, 3x1, 15x0, 15x0, 5x1, 12x0, 17x0, 8x1, 7x0, 19x0, 8x1, 5x0, 25x0, 3x1, 4x0, 10x0, 4x1, 6x0, 
                2x1, 5x0, 1, 4x0, 10x0, 1, 3, 3x1, 4x0, 6x1, 0, 2x1, 4x0, 10x0, 11x1, 2x0, 5x1, 4x0, 24x0, 3x1, 5x0, 
                32x0, 32x0, 32x0, 32x0, 32x0, 32x0, 32x0, 32x0, 32x0, 32x0, 32x0, 32x0`;
  }
...
function loadMapData(data) {
  if (data && data.length > 0) {
    ...
    gameStartTileX = mapData.startX;
    gameStartTileY = mapData.startY;
    gameExitTileX = mapData.exitX;
    gameExitTileY = mapData.exitY;
    selectImage(mapData.imageIndex);
    selectAudio(mapData.soundIndex);
  }
}
...
/**
 *   This is used for validating the maze to make sure that it is possible
 *   to complete it!
 *
 *   EasyStar.js
...
var EasyStar = (function (t) {
  ...
  function (t, n, e) {
    ...
    (o.js = function () {
      ...
      (this.setAcceptableTiles = function (t) {
        t instanceof Array
          ? (e = t)
          : !isNaN(parseFloat(t)) && isFinite(t) && (e = [t]);
      }),
        ...
        (this.avoidAdditionalPoint = function (t, n) {
          void 0 === p[n] && (p[n] = {}), (p[n][t] = 1);
        }),
        (this.stopAvoidingAdditionalPoint = function (t, n) {
          void 0 !== p[n] && delete p[n][t];
        }),
        ...
        (this.findPath = function (n, o, i, u, a) {
          var f = function (t) {
            h
              ? a(t)
              : setTimeout(function () {
          ...
          if (n === i && o === u) return void f([]);
          for (var p = t[u][i], d = !1, v = 0; v < e.length; v++)
            if (p === e[v]) {
              d = !0;
              break;
            }
          if (d === !1) return void f(null);
          var y = new r();
          (y.openList = new s(function (t, n) {
            return t.bestGuessDistance() - n.bestGuessDistance();
          })),
            (y.isDoneCalculating = !1),
            (y.nodeHash = {}),
            (y.startX = n),
            (y.startY = o),
            (y.endX = i),
            (y.endY = u),
            (y.callback = f),
            y.openList.push(P(y, y.startX, y.startY, null, c));
          var T = l++;
          return (g[T] = y), x.push(T), T;
        }),
        (this.cancelPath = function (t) {
          return t in g ? (delete g[t], !0) : !1;
        }),
        (this.calculate = function () {
          if (0 !== x.length && void 0 !== t && void 0 !== e)
            for (n = 0; O > n; n++) {
              ...
              if ("undefined" != typeof r)
                if (0 !== r.openList.size()) {
                  var i = r.openList.pop();
                  if (r.endX !== i.x || r.endY !== i.y)
                    (i.list = u),
                      i.y > 0 && m(r, i, 0, -1, c * F(i.x, i.y - 1)),
                      i.x < t[0].length - 1 &&
                        m(r, i, 1, 0, c * F(i.x + 1, i.y)),
                      i.y < t.length - 1 && m(r, i, 0, 1, c * F(i.x, i.y + 1)),
                      i.x > 0 && m(r, i, -1, 0, c * F(i.x - 1, i.y)),
                      b &&
                        (i.x > 0 &&
                          i.y > 0 &&
                          (T ||
                            (A(t, e, i.x, i.y - 1) && A(t, e, i.x - 1, i.y))) &&
                          m(r, i, -1, -1, f * F(i.x - 1, i.y - 1)),
                        i.x < t[0].length - 1 &&
                          i.y < t.length - 1 &&
                          (T ||
                            (A(t, e, i.x, i.y + 1) && A(t, e, i.x + 1, i.y))) &&
                    ...
                    var s = [];
                    s.push({ x: i.x, y: i.y });
                    for (var a = i.parent; null != a; )
                      s.push({ x: a.x, y: a.y }), (a = a.parent);
                    s.reverse();
                    var l = s;
                    r.callback(l), delete g[o], x.shift();
                  }
                } else r.callback(null), delete g[o], x.shift();
              else x.shift();
            }
        });
      var m = function (n, o, r, i, s) {
          var u = o.x + r,
            l = o.y + i;
          if ((void 0 === p[l] || void 0 === p[l][u]) && A(t, e, u, l, o)) {
            var c = P(n, u, l, o, s);
            void 0 === c.list
              ? ((c.list = a), n.openList.push(c))
              : o.costSoFar + s < c.costSoFar &&
                ((c.costSoFar = o.costSoFar + s),
                (c.parent = o),
                n.openList.updateItem(c));
          }
        },
        A = function (t, n, e, o, r) {
          ...
          for (var a = 0; a < n.length; a++) if (t[o][e] === n[a]) return !0;
          return !1;
        },
        E = function (t, n) {
        ...
        F = function (n, e) {
          return (v[e] && v[e][n]) || d[t[e][n]];
        },
        P = function (t, n, e, o, r) {
          if (void 0 !== t.nodeHash[e]) {
            if (void 0 !== t.nodeHash[e][n]) return t.nodeHash[e][n];
          } else t.nodeHash[e] = {};
          var s = L(n, e, t.endX, t.endY);
          if (null !== o) var u = o.costSoFar + r;
          else u = 0;
          var a = new i(o, n, e, u, s);
          return (t.nodeHash[e][n] = a), a;
        },
        L = function (t, n, e, o) {
          if (b) {
            var r = Math.abs(t - e),
              i = Math.abs(n - o);
            return i > r ? f * r + i : f * i + r;
          }
          var r = Math.abs(t - e),
            i = Math.abs(n - o);
          return r + i;
  ...
  function (t, n, e) {
    var o, r, i;
    (function () {
      var e, s, u, a, l, c, f, h, p, d, v, y, T, g, x;
      (u = Math.floor),
        (d = Math.min),
        (s = function (t, n) {
        ...
        (p = function (t, n, e, o, r) {
          var i;
          if ((null == e && (e = 0), null == r && (r = s), 0 > e))
            throw new Error("lo must be non-negative");
          for (null == o && (o = t.length); o > e; )
            (i = u((e + o) / 2)), r(n, t[i]) < 0 ? (o = i) : (e = i + 1);
          return [].splice.apply(t, [e, e - e].concat(n)), n;
        }),
        (c = function (t, n, e) {
          return null == e && (e = s), t.push(n), g(t, 0, t.length - 1, e);
        }),
        (l = function (t, n) {
          var e, o;
          return (
            null == n && (n = s),
            (e = t.pop()),
            t.length ? ((o = t[0]), (t[0] = e), x(t, 0, n)) : (o = e),
            o
          );
        }),
        (h = function (t, n, e) {
          var o;
          return null == e && (e = s), (o = t[0]), (t[0] = n), x(t, 0, e), o;
        }),
        (f = function (t, n, e) {
          var o;
          return (
        ...
        (T = function (t, n, e) {
          ...
          return (
            null == e && (e = s),
            (o = t.indexOf(n)),
            -1 !== o ? (g(t, 0, o, e), x(t, o, e)) : void 0
          );
        }),
        (v = function (t, n, e) {
          var o, r, i, u, l;
          if ((null == e && (e = s), (r = t.slice(0, n)), !r.length)) return r;
          for (a(r, e), l = t.slice(n), i = 0, u = l.length; u > i; i++)
            (o = l[i]), f(r, o, e);
          return r.sort(e).reverse();
        }),
        (y = function (t, n, e) {
          var o, r, i, u, c, f, h, v, y, T;
          if ((null == e && (e = s), 10 * n <= t.length)) {
            if (((u = t.slice(0, n).sort(e)), !u.length)) return u;
            for (
              i = u[u.length - 1], v = t.slice(n), c = 0, h = v.length;
              h > c;
              c++
            )
              (o = v[c]),
                e(o, i) < 0 &&
                  (p(u, o, 0, null, e), u.pop(), (i = u[u.length - 1]));
            return u;
          }
          for (
            a(t, e), T = [], r = f = 0, y = d(n, t.length);
            y >= 0 ? y > f : f > y;
            r = y >= 0 ? ++f : --f
          )
            T.push(l(t, e));
        ...
        (x = function (t, n, e) {
          var o, r, i, u, a;
          for (
            null == e && (e = s), r = t.length, a = n, i = t[n], o = 2 * n + 1;
            r > o;
        ...
        (e = (function () {
          ...
          return (
            ...
            (t.prototype.updateItem = function (t) {
              return T(this.nodes, t, this.cmp);
            }),
